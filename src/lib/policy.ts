import { HIGH_VALUE_CENTS, RETURN_WINDOW_DAYS } from './config';
import type { Order, PolicyResult, RefundReason } from './types';

const DAY_MS = 86_400_000;

// Base policy (SPEC §7.3, B1–B7). Pure; first matching rule wins.
export function evaluateRefund(order: Order | null, _reason: RefundReason, now: Date): PolicyResult {
  ...
  if (days > RETURN_WINDOW_DAYS.standard) {
    return { decision: 'deny', reason_code: 'OUTSIDE_WINDOW', refund_amount_cents: 0 };
  }
  if (order.amount_cents > HIGH_VALUE_CENTS) {
